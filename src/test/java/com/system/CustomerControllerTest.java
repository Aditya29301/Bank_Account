...
class CustomerControllerTest {
	...
	{
		List<Customer> customers = Arrays.asList(new Customer(1,"Aditya","Bangalore",null),new Customer(2,"Aadi","Bareilly",null));
		...
		mockMvc.perform(get("/getcustomers"))
				...
				.andExpect(jsonPath("$.data[0].customername").value("Aditya"))
				.andExpect(jsonPath("$.data[0].customerAddress").value("Bangalore"))
				.andExpect(jsonPath("$.data[1].customerid").value(2))
				.andExpect(jsonPath("$.data[1].customername").value("Aadi"))
				.andExpect(jsonPath("$.data[1].customerAddress").value("Bareilly"));
	...
	public void testUpdatebById() throws Exception
	{
		...
		uc.setCustomername("Aadi");
		uc.setCustomerAddress("Bareilly");
		...
		mockMvc.perform(put("/update/{id}",1)
				...
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.data.customername").value("Aadi"))
				.andExpect(jsonPath("$.data.customerAddress").value("Bareilly"));
